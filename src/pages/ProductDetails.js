        ...
        {/* {!data.isVerified && ( */}
            ...
            {data.composition && data.composition.length > 0 && (
                ...
                {data.composition.map((item,index) =>(
                    <p className='text-black text-[16px] font-normal capitalize font-Roboto'><span className='text-primary  font-semibold text-RobotoBold '>{index + 1}.</span>{item.name && item.name}</p>
                ))}
                <hr className='my-2'/>
                </>
            )}
